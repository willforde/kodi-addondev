...
def getLocalizedString(id):
    ...
    """
    string = session.get_addon("resource.language.en_gb").strings[id]
...
def makeLegalFilename(filename, fatX=True):
    ...
    if fatX:
        ...
        return os.path.join(path, _normalize_filename(filename))
    else:
...
def translatePath(path):
    ...
    """
    return session.translate_path(path)
...
class Keyboard(object):
    ...
    def __init__(self, default="", heading="", hidden=False):
        self._xbmcgui = __import__("xbmcgui")
    ...
    def doModal(self, autoclose=0):
        ...
        dialog = self._xbmcgui.Dialog()
        self._input = dialog.input(self._heading, self._default,
                                   option=self._xbmcgui.ALPHANUM_HIDE_INPUT if self._hidden else 0)
...
class PlayList(object):
    ...
    def __init__(self, playlist=PLAYLIST_VIDEO):
        self.data = session.data.playlist  # type: List[str]
        self._xbmcgui = __import__("xbmcgui")
    ...
    def add(self, url, listitem=None, index=None):
        ...
        """
        if listitem is None:
            listitem = self._xbmcgui.ListItem()
        ...
        if index is None:
            ...
            self.data.insert(index, listitem)
    ...
    def unshuffle(self):
        ...
        del self.data[:]
        self.data.extend(self.playlistItems)
